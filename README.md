...
### Изменение настроек
* В папке `\levels\` можно изменять уровни и создавать собственные. Размерность поля не должна быть изменена, а именно 40 блоков в ширину, 30 в высоту. `' '(Spacebar)` - пустое место на поле, `1` - расположение змейки (она должна быть неразрывной), `9` - стена.
